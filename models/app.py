...
def delete_schedule():
    ...
    # Delete physical image files from the uploads directory
    try:
        upload_dir = app.config['UPLOAD_FOLDER']
        # The email was sanitized using secure_filename when saving, so we must sanitize it here to match!
        secured_email = secure_filename(user_email)
        prefix = f"{secured_email}_"
        ...
        print(f"Error deleting physical files for {user_email}: {e}")
    ...
    return jsonify({"success": True, "message": "Schedule and images deleted permanently"})
